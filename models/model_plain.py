...
from models.loss import CharbonnierLoss, SID_Loss
...
class ModelPlain(ModelBase):
    ...
    def __init__(self, opt):
        ...
        if self.opt_train['E_decay'] > 0:
            self.netE = define_G(opt).to(self.device).eval()   #net_E是net_G的评价模式
    ...
    def load(self):
        ...
        if self.opt_train['E_decay'] > 0:     #E_decay是学习率衰减指数
    ...
    def save(self, iter_label):
        ...
        if (self.opt_train['E_decay'] > 0) and ((iter_label % (self.opt_train['checkpoint_save'] * 20)) == 0):
    ...
    def define_loss(self):
        ...
        elif G_lossfn_type == 'pf_edge_dloss':
            self.G_lossfn = SID_Loss(self.opt_train['lam_vgg'], self.opt_train['lam_edge']).to(
                self.device)
