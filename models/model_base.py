...
'''
ModelBase是model_plain的父类，用于定义，存储，输出模型
opt是Option文件打开之后的字典
'''
class ModelBase():    #一个存储和使用模型的工具
    def __init__(self, opt):
        self.opt = opt                         # opt
        self.save_dir = opt['path']['patterned_models']  # save patterned_models
        self.device = torch.device('cuda' if opt['gpu_ids'] is not None else 'cpu')
        self.is_train = opt['is_train']        # training or not
        self.schedulers = []                   # schedulers  里面是optimizer
    ...
    def current_learning_rate(self):            #获取当前的learning_rate
        return self.schedulers[0].get_lr()[0]

    def requires_grad(self, model, flag=True):  #设置本模型是否需要梯度
    ...
    def model_to_device(self, network):
        """Model to device. It also warps patterned_models with DistributedDataParallel
        or DataParallel.
        Args:
            network (nn.Module)
        """
    ...
    # ----------------------------------------
    def save_network(self, save_dir, network, network_label, iter_label):    #存模型
