...
from data.data_utils import stn, mod_flash, get_dshift_pattern,get_pattern_raw
...
class DatasetPF(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        print('Dataset: Denosing on AWGN with fixed sigma. Only dataroot_H is needed.')  #AWGN 高斯加权白噪声
        self.opt = opt
        ...
        self.disp_dir = self.data_dir.replace('dist_cleanpass', 'disparity')
        ...
        self.pattern_raw = np.zeros((self.pattern.shape[0] * 2, self.pattern.shape[1] * 2))  # 2*2扩大
        ...
        self.pattern_raw = self.pattern_raw.astype(np.float32)     #RGGB 生成RAW数据
        self.pattern_boost = opt['pattern_boost']

    def get_pattern_raw2(self,tensor):
        raw = torch.zeros(tensor.shape)
        raw[::2, ::2] = tensor[::2, ::2, 1]
        raw[1::2, 1::2] = tensor[1::2, 1::2, 1]
        raw[::2, 1::2] = tensor[::2, 1::2, 3]
        raw[1::2, ::2] = tensor[1::2, ::2, 0]
        return raw
    ...
    def __getitem__(self, idx):
        flash, depth = self.get_ft3d_fd(idx)            #读取RGB和pfm数据
        ...
        img_flash_crop = self.get_pattern_raw2(img_flash_crop)
        H_path = '{}'.format(idx)
        L_path = H_path
