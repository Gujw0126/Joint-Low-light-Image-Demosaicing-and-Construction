...
def readPFM(file):
    ...
    header = file.readline().rstrip()     #删除空白符
    if header.decode("ascii") == 'PF':    #PF/Pf是否是彩色数据
        color = True
    ...
    dim_match = re.match(r'^(\d+)\s(\d+)\s$', file.readline().decode("ascii"))   #正则表达式
    #PF后是Dimensions Line维度行包含两个正十进制整数，用空格分隔。第一个是图像的宽度；第二个是高度。
    ...
    if scale < 0: # little-endian      小端存储
        endian = '<'
        scale = -scale
    else:
        endian = '>' # big-endian     大端存储

    data = np.fromfile(file, endian + 'f')
    shape = (height, width, 3) if color else (height, width)

    data = np.reshape(data, shape)            #返回np
    data = np.flipud(data)
    return data, scale
...
def readFlow(name):
    if name.endswith4('.pfm') or name.endswith('.PFM'):
